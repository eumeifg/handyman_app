    ...
    await flutterWave.charge().then((value) {
      if (value.status == "successful") {
        ...
        verifyPayment(transactionId: value.transactionId.validate(), flutterWaveSecretKey: flutterWaveSecretKey).then((v) {
          if (v.status == "success") {
            savePayment(data: selectedPricingPlan, paymentMethod: PAYMENT_METHOD_FLUTTER_WAVE, paymentStatus: SERVICE_PAYMENT_STATUS_PAID, txtId: value.transactionId.validate()).catchError(onError);
          } else {
