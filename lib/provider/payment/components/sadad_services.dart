...
import 'package:sun3ah_provider/provider/payment/components/payment_webview_screen.dart';
...
class SadadServices {
  ...
  Future<void> createInvoice(BuildContext context, {required String accessToken}) async {
    Map<String, dynamic> req = {
      ...
      "cellnumber": appStore.userContactNumber.validate(),
      "invoicedetails": [
        {
    ...
    sadadCreateInvoice(request: req, sadadToken: accessToken).then((value) async {
      ...
      if (res.validate().isNotEmpty) {
        savePayment(data: planData, paymentMethod: PAYMENT_METHOD_SADAD_PAYMENT, paymentStatus: SERVICE_PAYMENT_STATUS_PAID, txtId: res);
      } else {
