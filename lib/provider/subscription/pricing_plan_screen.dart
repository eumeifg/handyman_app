...
import 'package:sun3ah_provider/provider/payment/payment_screen.dart';
...
class _PricingPlanScreenState extends State<PricingPlanScreen> {
  ...
  Widget build(BuildContext context) {
    return RefreshIndicator(
      ...
      child: Scaffold(
        appBar: appBarWidget(languages!.lblPricingPlan, backWidget: BackWidget(), elevation: 0, color: primaryColor, textColor: Colors.white),
        body: Stack(
          fit: StackFit.expand,
          children: [
            SingleChildScrollView(
              ...
              child: Column(
                children: [
                  42.height,
                  Text(languages!.lblSelectPlan, style: boldTextStyle(size: 18)),
                  24.height,
                  ListView.builder(
                    shrinkWrap: true,
            ...
            if (selectedPricingPlan != null)
              Positioned(
                ...
                right: 16,
                child: AppButton(
                  child: Text(selectedPricingPlan!.identifier == FREE ? languages!.lblProceed.toUpperCase() : languages!.lblMakePayment.toUpperCase(), style: boldTextStyle(color: white)),
                  color: primaryColor,
                  onTap: () async {
            ...
            Observer(
              builder: (_) {
                return BackgroundComponent(
                  text: languages!.noSubscriptionPlan,
                ).center().visible(!appStore.isLoading && pricingPlanList.isEmpty && !hasError);
              },
            ),
