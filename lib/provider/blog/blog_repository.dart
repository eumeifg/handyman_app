...
import 'package:sun3ah_provider/provider/blog/model/blog_detail_response.dart';
...
Future<List<BlogData>> getBlogListAPI({int? page, required List<BlogData> blogData, Function(bool)? lastPageCallback}) async {
  ...
  BlogResponse res = BlogResponse.fromJson(
    await handleResponse(await buildHttpResponse('blog-list?provider_id=${appStore.userId}&per_page=$PER_PAGE_ITEM&page=$page', method: HttpMethod.GET)),
  );
...
Future<BaseResponseModel> deleteBlogAPI(int? id) async {
  return BaseResponseModel.fromJson(await handleResponse(await buildHttpResponse('blog-delete/$id', method: HttpMethod.POST)));
}
//endregion
